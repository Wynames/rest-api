...
export async function GET(request) {
    ...
    // 2. Ambil user_id SEKALI untuk logging
    ...
    const insertLog = async (statusCode) => {
        ...
        await supabaseAdmin.from('api_logs').insert([
            {
                user_id: userId,
                endpoint: '/api/anime/kucing-detail',
                method: 'GET',
    ...
    if (!postId || isNaN(Number(postId))) {
        await insertLog(400);
        return NextResponse.json(
            { success: false, message: 'Parameter id diperlukan dan harus berupa angka yang valid.' },
            { status: 400 }
        );
    }
    ...
    try {
        const data = await client.getPostDetails(postId);
        await insertLog(200);
        return NextResponse.json(
            { success: true, data },
            { status: 200 }
        ...
        await insertLog(500);
        return NextResponse.json(
            { success: false, message: err.message || 'Gagal mengambil detail post.' },
            { status: 500 }
        );
    }
}
